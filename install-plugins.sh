...
doDownload() {
    ...
    retry_command curl "${CURL_OPTIONS:--ksSfL}" --connect-timeout "${CURL_CONNECTION_TIMEOUT:-20}" --retry "${CURL_RETRY:-5}" --retry-delay "${CURL_RETRY_DELAY:-0}" --retry-max-time "${CURL_RETRY_MAX_TIME:-60}" "$url" -o "$jpi"
    return $?
}
...
main() {
    ...
    if curl -kfsL -o /dev/null "$JENKINS_UC/$jenkinsVersion"; then
